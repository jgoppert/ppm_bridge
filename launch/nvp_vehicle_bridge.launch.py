"""
This launch file is intended to initiate ppm bridge for a 3-channel fixed-wing UAV, UMX Night Vapor
...
ARGUMENTS = [
    # launch arguments
    DeclareLaunchArgument(
        "sim", default_value=["false"], description="use with simulation"
    ),
    DeclareLaunchArgument(
        "rviz",
        default_value="false",
...
def generate_launch_description():
    ...
    rviz_node = Node(
        ...
        arguments=[
            ...
            [
                PathJoinSubstitution(
                    [FindPackageShare("electrode"), "config", "bridge"]
                ),
                ".rviz",
            ],
            "--ros-args",
    ...
    vehicle_description = LaunchDescription(
        ARGUMENTS
        + [
            joy,
            bridge,
    ...
    vehicle_1 = GroupAction(
        actions=[PushROSNamespace(LaunchConfiguration("vehicle")), vehicle_description]
    )
